...
export default function LeaderboardCard({
  ...
  return (
    <div
      className={`bg-white rounded-2xl shadow-xl border border-[#3F72AF]/30 overflow-hidden ${
        compact ? "" : "animate-scale-in"
      }`}
    >
      {/* Header */}
        ...
        <>
          <div className="p-3 sm:p-4 space-y-2 sm:space-y-3">
            {topPlayers.map((player, index) => (
              <div
                key={player.rank}
                className={`group flex items-center justify-between p-3 sm:p-4 rounded-xl border transition-all duration-300 hover:shadow-md hover:scale-102 ${getRankBg(
                  player.rank
              ...
              >
                ...
                <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                  ...
                  {player.badge ? (
                    ...
                    <div
                      className={`flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-gradient-to-r ${getRankColor(
                        player.rank
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p
                        className={`font-semibold text-[#112D4E] text-sm sm:text-base truncate ${
                ...
                <div className="flex-shrink-0 ml-2 text-right">
                  ...
                  >
                    ...
                    <p className="text-xs text-gray-600 mt-0.5">
                      {player.totalCorrect} benar •{" "}
                      {player.bestTime
                        ? `${Math.floor(player.bestTime / 60)}:${(
                            player.bestTime % 60
                          )
                            .toString()
                            .padStart(2, "0")}`
